...
def adjust_text_embeddings(embeddings, attention_mask, azimuth, polar, guidance_opt):
    ...
    text_z_, mask_z_, weights_ = get_pos_neg_text_embeddings(embeddings, attention_mask, azimuth, polar, guidance_opt)
    K = max(K, weights_.shape[0])
    text_z_list.append(text_z_)
    mask_z_list.append(mask_z_)
    ...
    if guidance_opt.is_PixArt:
        for i in range(K):
            for mask_z in mask_z_list:
                mask_attention.append(mask_z[i] if i < len(mask_z) else mask_z[0])
        mask_attention = torch.stack(mask_attention, dim=0)
    else:
    ...
    # breakpoint()
    return text_embeddings, mask_attention, weights

def get_pos_neg_text_embeddings(embeddings, attention_mask, azimuth_val, polar_val, opt):
    mask_z = torch.tensor([])
    if opt.is_PixArt:
        if azimuth_val >= -90 and azimuth_val < 90:
            ...
            start_attention_z = attention_mask['front']
            end_attention_z = attention_mask['side']        
            pos_z = r * start_z + (1 - r) * end_z
            pos_attention_z = r * start_attention_z + (1 - r) * end_attention_z
            text_z = torch.cat([pos_z, embeddings['front'], embeddings['side']], dim=0)
            mask_z = torch.cat([pos_attention_z, attention_mask['front'], attention_mask['side']], dim = 0)
            ...
            start_attention_z = attention_mask['side']
            end_attention_z = attention_mask['back'] 
            pos_z = r * start_z + (1 - r) * end_z
            pos_attention_z = r * start_attention_z + (1 - r) * end_attention_z
    ...
    # breakpoint()
    return text_z, mask_z, weights.to(text_z.device)
...
def prepare_embeddings(guidance_opt, guidance):
    if guidance_opt.is_PixArt:
        ...
        embeddings['default'], attention_mask['default'] = guidance.get_text_embeds([guidance_opt.text])
        embeddings['uncond'], attention_mask['uncond'] = guidance.get_text_embeds([guidance_opt.negative])
        
        if guidance_opt.use_overhead:
            for d in ['front', 'side', 'back', 'overhead']:
                embeddings[d], attention_mask[d] = guidance.get_text_embeds([f"{guidance_opt.text}, {d} view"])
        else:
            for d in ['front', 'side', 'back']:
                embeddings[d], attention_mask[d] = guidance.get_text_embeds([f"{guidance_opt.text}, {d} view"])
        embeddings['inverse_text'], attention_mask['inverse_text'] = guidance.get_text_embeds(guidance_opt.inverse_text)
    else:
        embeddings = {}
        attention_mask = {}
        # text embeddings (stable-diffusion) and (IF)
...
def guidance_setup(guidance_opt, iterations):
    if guidance_opt.guidance=="SD" and not guidance_opt.is_PixArt:
        if guidance_opt.is_LCM:
            ...
            guidance = StableDiffusion(guidance_opt.g_device, guidance_opt.fp16, guidance_opt.vram_O, 
                                    ...
                                    ddim_inv=guidance_opt.ddim_inv, use_control_net=guidance_opt.use_control_net,
                                    textual_inversion_path = guidance_opt.textual_inversion_path,
                                    LoRA_path = guidance_opt.LoRA_path,
                                    guidance_opt=guidance_opt, iterations = iterations)
    #     else:
    #         from guidance.sd_utils_ism_sds import StableDiffusion
    #         guidance = StableDiffusion(guidance_opt.g_device, guidance_opt.fp16, guidance_opt.vram_O, 
    #                                 guidance_opt.t_range, guidance_opt.max_t_range, 
    #                                 num_train_timesteps=guidance_opt.num_train_timesteps, 
    #                                 ddim_inv=guidance_opt.ddim_inv, use_control_net=guidance_opt.use_control_net,
    ...
    # elif guidance_opt.guidance=="SD" and guidance_opt.is_PixArt:
    #     from guidance.sd_utils_pixArt import StableDiffusion
    #     guidance = StableDiffusion(guidance_opt.g_device, guidance_opt.fp16, guidance_opt.vram_O, 
    #                                guidance_opt.t_range, guidance_opt.max_t_range, 
    #                                num_train_timesteps=guidance_opt.num_train_timesteps, 
    #                                ddim_inv=guidance_opt.ddim_inv, use_control_net=guidance_opt.use_control_net,
    ...
    embeddings, attention_mask = prepare_embeddings(guidance_opt, guidance)
    return guidance, embeddings, attention_mask

def training(dataset, opt, pipe, gcams, guidance_opt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, save_video, save_dir, save_mesh_mode):
    first_iter = 0
    tb_writer = prepare_output_and_logger(dataset, save_dir)
    gaussians = GaussianModel(dataset.sh_degree)
    scene = Scene(dataset, gcams, pipe, gaussians)
    ...
    guidance, embeddings, attention_mask = guidance_setup(guidance_opt, opt.iterations) 
    # if guidance_opt.VSD:
    #     unet, unet_optimizer, unet_scheduler = unet_setup(guidance_opt,guidance)
    # else:
    unet = None
    viewpoint_stack = None
    viewpoint_stack_around = None
    viewpoint_overhead_stack = None
    ...
    for iteration in range(first_iter, opt.iterations + 1):  
        ...
        if guidance_opt.overhead_loss_ratio != 0 and not viewpoint_overhead_stack:
            viewpoint_overhead_stack = scene.getCertainTrainCameras(azimuth = 0, polar = 0).copy()
        ...
        if guidance_opt.is_PixArt:
            mask_z_inverse = torch.cat([attention_mask['uncond'],attention_mask['inverse_text']], dim=0)
        
        
        render_normal = False
        if guidance_opt.use_control_net and (guidance_opt.controlnet_type == 'normal' or 
        guidance_opt.controlnet_type == 'normal&depth'):
            render_normal = True
        ...
        for i in range(C_batch_size):
            ...
            if viewpoint_overhead_stack:
                # print('nononononoaaaaaaaaaaaaaaa')
                try:
                    viewpoint_overhead_cam = viewpoint_overhead_stack.pop(randint(0, len(viewpoint_overhead_stack)-1))            
                except:
                    viewpoint_overhead_stack = scene.getCertainTrainCameras(azimuth = 0, polar = 0).copy()
                    viewpoint_overhead_cam = viewpoint_overhead_stack.pop(randint(0, len(viewpoint_overhead_stack)-1))
            ...
            polar = viewpoint_cam.delta_polar
            # print('polar',polar)
            
            text_z = [embeddings['uncond']]
            if guidance_opt.is_PixArt:
                mask_z = [attention_mask['uncond']]
            else:
                mask_z = []
            pose = torch.tensor(viewpoint_cam.pose).to(guidance_opt.device)


            if guidance_opt.perpneg:
                text_z_comp, mask_z_comp, weights = adjust_text_embeddings(embeddings, attention_mask, azimuth, polar, guidance_opt)
                text_z.append(text_z_comp)
                mask_z.append(mask_z_comp)
                ...
                if guidance_opt.is_PixArt:
                    ...
                    mask_z.append(r * start_attention_z + (1 - r) * end_attention_z)
                else:                 
                    if azimuth >= -90 and azimuth < 90:
                        if azimuth >= 0:
                            r = 1 - azimuth / 90
            ...
            if guidance_opt.is_PixArt:
                mask_z = torch.cat(mask_z, dim=0)
                mask_z_.append(mask_z)
            ...
            if guidance_opt.is_PixArt:
                resoltion = 1024
            if guidance_opt.resolution_warm_up:
                if iteration < 250:
                    resoltion = resolution / 4
                if iteration >= 250 and iteration < 500:
                    resoltion = resolution / 2
                
            render_pkg = render(viewpoint_cam, gaussians, pipe, background, 
                                sh_deg_aug_ratio = dataset.sh_deg_aug_ratio, 
                                bg_aug_ratio = dataset.bg_aug_ratio, 
                                shs_aug_ratio = dataset.shs_aug_ratio, 
                                scale_aug_ratio = dataset.scale_aug_ratio, return_normal = render_normal, resolution = resolution)
            ...
            if render_normal:
                ...
                # normal_folder = os.path.join(dataset._model_path,"normal/")
                # if not os.path.exists(normal_folder):
                #     os.makedirs(normal_folder)  # makedirs
                # torchvision.utils.save_image(normal, os.path.join(normal_folder, '{0:05d}'.format(iteration) + ".png"))
        ...
        poses = poses.view(C_batch_size, 16)
        ...
        _aslatent = False
        # print(guidance_opt.x0_iters)
        if iteration <= guidance_opt.x0_iters and (random.random() < opt.as_latent_ratio):
            # print('yes', opt.as_latent_ratio)
            _aslatent=True
        if iteration > opt.use_control_net_iter and (random.random() < guidance_opt.controlnet_ratio):
            use_control_net = True
            
        if guidance_opt.perpneg:
            step_ratio = (iteration - guidance_opt.x0_iters) / (opt.iterations - guidance_opt.x0_iters)
            loss= guidance.train_step_perpneg(torch.stack(text_z_, dim=1), images, 
                                            ...
                                            guidance_opt=guidance_opt,as_latent=_aslatent, embedding_inverse = text_z_inverse, step_ratio = step_ratio)
        ...
        # print('opt.lambda_bin',opt.lambda_bin)
        ...
        with torch.no_grad():
            ...
            if (iteration in testing_iterations):
                if save_video:
                    video_inference(iteration, scene, render, (pipe, background), start_time = start_time)

            if (iteration in saving_iterations):
                print("\n[ITER {}] Saving Gaussians".format(iteration))
                if save_mesh_mode != '':
                    scene.save(iteration, bg_color = background)
                    scene.save(iteration, bg_color = background, mode = save_mesh_mode)
                else:
                    scene.save(iteration, bg_color = background)
            ...
            if iteration < opt.densify_until_iter:
                ...
                if (iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0) or iteration == opt.densify_init_iter:
                    # print('desify',iteration)
            ...
            if (iteration in checkpoint_iterations):
                ...
                torch.save((gaussians.capture(), iteration), scene.args._model_path + "/chkpnt" + str(iteration) + ".pth")
...
def prepare_output_and_logger(args, save_dir):    
    if not args._model_path:
        if os.getenv('OAR_JOB_ID'):
            unique_str=os.getenv('OAR_JOB_ID')
        else:
            unique_str = str(uuid.uuid4())
        args._model_path = os.path.join(save_dir, args.workspace)
    ...
    if args.opt_path is not None:
        config_path = os.path.join(save_dir, args._model_path, 'config.yaml')
        os.system(' '.join(['cp', args.opt_path, '"'+config_path+'"']))
...
def video_inference(iteration, scene : Scene, renderFunc, renderArgs, start_time = 0):
    sharp = T.RandomAdjustSharpness(3, p=1.0)
    
    # time_iter = (time.time() - start_time) / 60
    # save_folder = os.path.join(scene.args._model_path,"videos/{}_iteration_time_{}".format(iteration, time_iter))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, default = '/media/data1/suruij/output')
    parser.add_argument("--save_mesh_mode", type=str, default = '') # 'geo' or 'geo+tex'
    parser.add_argument("--add_param", type=ast.literal_eval, default = "{}")
    ...
    print(args.add_param)

    if args.opt is not None:
        print(args.opt)
        with open(args.opt) as f:
            opts = yaml.load(f, Loader=yaml.FullLoader)
        if len(args.add_param)!=0:
            for k in args.add_param.keys():
                k_split = k.split('.')
                if len(k_split) == 1:
                    opts[k_split[0]] = args.add_param[k]
                else:
                    # print(opts[k_split[0]][k_split[1]])
                    opts[k_split[0]][k_split[1]] = args.add_param[k]
        ...
        gp.device = args.device
        
    gcp.prompt = gp.text
    op.stage1_iters = gp.x0_iters

    # save iterations
    test_iter = [k * op.iterations // args.test_ratio for k in range(1, args.test_ratio)] + [op.iterations] + [1]
    # test_iter = [1, 100, 500, 925, 1325, 1725, 2525, 2850, 3200, 3525, 4000]
    ...
    print('args.test_ratio',args.test_ratio)
    ...
    training(lp, op, pp, gcp, gp, args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.save_video, args.save_dir, args.save_mesh_mode)
