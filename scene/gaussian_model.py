...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        scales = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)     #每个点在X, Y, Z方向上的尺度
    ...
    def training_setup(self, training_args):
        self.percent_dense = training_args.percent_dense     #是否分割/合并高斯核的参数
        self.xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.denom = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")

        l = [
            {'params': [self._xyz], 'lr': training_args.position_lr_init * self.spatial_lr_scale, "name": "xyz"},
            {'params': [self._features_dc], 'lr': training_args.feature_lr, "name": "f_dc"}, #第一个球谐系数, 球谐系数用来表示RGB颜色
            {'params': [self._features_rest], 'lr': training_args.feature_lr / 20.0, "name": "f_rest"},      #其余球谐系数
            {'params': [self._opacity], 'lr': training_args.opacity_lr, "name": "opacity"},     #透明度
            {'params': [self._scaling], 'lr': training_args.scaling_lr, "name": "scaling"},       #尺度
            {'params': [self._rotation], 'lr': training_args.rotation_lr, "name": "rotation"},     #旋转
        ...
        self.xyz_scheduler_args = get_expon_lr_func(lr_init=training_args.position_lr_init*self.spatial_lr_scale,
                                                    ...
                                                    lr_warmup_factor=training_args.lr_warmup_factor,
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):    #scene_extent: default_radius
        # Extract points that satisfy the gradient condition
